...
varNames = X_original.Properties.VariableNames;
%%
% n = 0.5*q
ratio = [8 2 1 0.5];
ind = 4;
...
for p = 1:7
    for run = 1:5
        ...
        NRSME1(run) = sqrt(mean((X_true{:,index_numeric} - X_imputed{:,index_numeric}).^2)/var(X_true{:,index_numeric}));
...
errorbar(1:1:7,str2double(string(NRSME)),str2double(string(sd_NRSME)),'-k','MarkerSize',7,...
    'Marker','*','MarkerEdgeColor','black','MarkerFaceColor','black','LineWidth',1)
xlim([1 7])
xticks([1 2 3 4 5 6 7])
xticklabels({'10%','20%','30%','40%','50%','60%','70%'})
title({'MissForest imputer method on numerical and categorical data';'12 Rows, 24 Columns';'15 Numeric features, 9 Text features'})
xlabel('Percentage of missing data')
ylabel('NRSME')
saveas(gcf,'fig4_NRSME.png')
hold off
%%
% Error plot for categorical data
errorbar(1:1:7,str2double(string(PFC)),str2double(string(sd_PFC)),'-k','MarkerSize',7,...
    'Marker','*','MarkerEdgeColor','black','MarkerFaceColor','black','LineWidth',1)
xlim([1 7])
xticks([1 2 3 4 5 6 7])
xticklabels({'10%','20%','30%','40%','50%','60%','70%'})
