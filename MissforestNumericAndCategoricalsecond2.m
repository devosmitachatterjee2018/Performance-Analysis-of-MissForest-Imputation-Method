...
array_numeric = varfun(@isnumeric,X_original,'output','uniform');
array_datetime = varfun(@isdatetime,X_original,'output','uniform');
index_numeric = find(array_numeric==1);
index_datetime = find(array_datetime==1);
index_nottext = [index_numeric index_datetime];
X_original = removevars(X_original,index_nottext);
%%
% n = 2*q
ratio = [8 2 1 0.5];
ind = 2;
...
title({'MissForest imputer method on categorical data';'18 Rows, 9 Columns';'9 Text features'})
xlabel('Percentage of missing data')
ylabel('PEC')
saveas(gcf,'fig2cat_PFC.png')